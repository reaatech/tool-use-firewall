...
- **Audit `sidecar` output** now forwards events to an HTTP log aggregator/SIEM (using `endpoint` + optional `api_key_env` Bearer auth) and/or appends them to a local file (`path`) — previously the `endpoint`/`api_key_env` fields were ignored. Delivery is best-effort and non-blocking. A `sidecar` output requires an `endpoint` and/or a `path`; a `file` output requires a `path`.
