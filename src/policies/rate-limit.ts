...
/**
 * Rate limiter middleware with global, per-tool, and per-session limits.
...
export class RateLimiter implements Middleware {
  ...
  constructor(config: RateLimitConfig) {
    ...
    this.sessionTtlMs = 3600000; // 1 hour
  ...
  private getToolLimiter(toolName: string): TokenBucket {
    ...
    if (!this.toolLimiters.has(toolName)) {
      const config = this.toolConfigs[toolName] ?? this.defaultToolConfig; // eslint-disable-line security/detect-object-injection
  ...
  private evictStaleToolLimiters(): void {
    ...
    // Hard eviction: remove oldest entries when over capacity
