...
import type { ValidationRule } from '../config/schema.js';
 ...
 * Supports regex patterns, shell safety, SQL safety, and custom validators.
 */
export class ArgumentValidator implements Middleware {
  ...
  constructor(
    ...
    sqlConfig?: ConstructorParameters<typeof SQLValidator>[0],
  ) {
    this.rules = rules;
  ...
  private registerDefaultValidators(): void {
    this.validators.set('shell_safe', (value: unknown) => {
      ...
      // Best-effort shell-injection screen. Not a substitute for never passing
      // user input to a shell — pass argv arrays to spawn() instead.
      const dangerous = [';', '&&', '||', '|', '`', '$(', '${', '\n', '\r', '>', '<', '\\'];
    ...
    // Legacy sql_safe validator (simple regex-based)
    // When sqlValidator is configured, the dedicated SQLValidator class is used instead
