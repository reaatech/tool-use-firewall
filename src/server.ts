...
import { createRequestContext } from './middleware/context.js';
import { InterceptorPipeline } from './middleware/interceptor.js';
import { PolicyEngine } from './policies/engine.js';
import { RateLimiter } from './policies/rate-limit.js';
import { CostTracker } from './policies/cost-tracker.js';
import { ArgumentValidator } from './policies/validator.js';
import { ReadOnlyCheck } from './policies/read-only.js';
import { AuditLogger } from './audit/index.js';
import { loadPolicyConfig } from './config/index.js';
import type { PolicyConfig } from './config/schema.js';
import { ApprovalRequiredError, FirewallError, RateLimitError } from './utils/errors.js';
...
/**
 * MCP Proxy Server that intercepts tool calls and enforces policies.
 * Spawns the upstream MCP server via stdio and proxies JSON-RPC messages.
 */
export class MCPProxyServer {
  ...
  async stop(): Promise<void> {
    // Send SIGTERM, then SIGKILL after a 2s grace period so we don't leave
    // the upstream draining after we're gone.
  ...
  private async handleDownstreamMessage(line: string): Promise<void> {
    ...
    // Non-tool methods pass through directly
    ...
    // Validate sessionId length to prevent memory abuse
    const sessionId =
    ...
    // Extract bypass token from request metadata if present
    ...
    try {
      ...
      if (!result.allowed) {
        // For APPROVAL_REQUIRED we defer the audit log to the catch block so that
        // requestApproval() can attach its approvalId; otherwise we'd emit two
        // audit events for the same logical decision.
        if (result.action === 'APPROVAL_REQUIRED' && this.approvalWorkflow) {
          await this.approvalWorkflow.requestApproval(context);
          // requestApproval always throws — control jumps to the catch.
        }
      ...
      // Forward to upstream and capture response.
      // We pass the original id through so it can be restored on the way back —
      // upstream sees the proxy's internal id, but the agent gets its own id back.
  ...
  private handleUpstreamMessage(line: string): void {
    ...
    if (id !== undefined && id !== null && this.pendingResponses.has(id)) {
      ...
      // Notification or unsolicited message — pass through
  ...
  private forwardToUpstreamAndWait(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pendingResponses.set(id, (value: unknown) => {
        clearTimeout(timeout);
        // Restore the agent's original id so JSON-RPC correlation works downstream.
