...
import { DEFAULT_REDACTION_PATTERNS, redact, type RedactionPattern } from '../utils/redactor.js';
...
export interface AuditLoggerOptions {
  config?: AuditConfig;
  /** When true, suppress emission. Defaults to NODE_ENV === 'test'. */
  silent?: boolean;
}

/**
 * Audit logger with redaction support.
 * Writes to stderr (never stdout) to avoid corrupting MCP JSON-RPC streams.
 * Honors policy `audit` config: level filter, optional file output, redaction toggle.
 */
export class AuditLogger {
  ...
  constructor(options: AuditLoggerOptions = {}) {
    ...
    // File outputs are validated and opened. `stdout` is forbidden — it would
