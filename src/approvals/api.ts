...
import { TokenBucket } from '../policies/rate-limit.js';
...
/**
 * HTTP API for managing approvals.
 * Runs alongside the MCP proxy to provide external approval interfaces.
 */
export function createApprovalApi(workflow: ApprovalWorkflow, apiKey: string): express.Application {
  ...
  app.use((req, res, next) => {
    // /health is unauthenticated to support liveness probes.
    if (req.path === '/health') {
      next();
      return;
    }
