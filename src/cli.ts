...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--upstream' || arg === '-u') {
      ...
      // After --upstream <cmd>, collect upstream args until either a known
      // firewall flag or a literal `--` (which we then drop).
    ...
    // Unknown flag — bail rather than silently swallow.
